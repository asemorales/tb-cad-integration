...
\caption{Feature Presence in Reviewed TB CAD Products and the Proposed Layer}
...
\footnotesize
\item Sources: official product documentation for qXR~\cite{qxr_docs}, CAD4TB+~\cite{cad4tb_product}, Lunit INSIGHT CXR~\cite{lunit_product}, Annalise Enterprise CXR~\cite{annalise_product}, and Siemens AI-Rad Companion~\cite{siemens_product}.
\item $\checkmark$ = documented feature; blank = absent or no clear evidence in the reviewed materials.
\end{tablenotes}
\end{threeparttable}
\end{table*}
...
\subsection{Tuberculosis Classification}
...
All models are trained on \textbf{TBX11K}, a public dataset of 11,200 chest radiographs with image-level class labels (\textit{healthy}, \textit{sick non-TB}, \textit{TB}) and expert-annotated bounding boxes for active-TB and obsolete (latent) TB regions~\cite{liu2020tbx11k}. We use the simplified, classification-oriented redistribution of TBX11K, the \texttt{tbx11k-simplified} release.\footnote{\url{https://www.kaggle.com/datasets/vbookshelf/tbx11k-simplified}} TBX11K was selected because (i) it is paired with reference baselines in the same publication, so reported metrics transfer cleanly; (ii) it supports reproducible train/validation/test evaluation; and (iii) it is the largest publicly available TB-specific CXR dataset with both image-level labels and region-level annotations. The image-level labels are used for the classification experiments in this section; the same dataset's bounding-box annotations are used for the localization experiments of Section~\ref{sec:tb_localization}.
...
\subsection{Tuberculosis Localization (Object Detection)}
...
Localization uses the same TBX11K data~\cite{liu2020tbx11k}, here with the region-level bounding-box annotations rather than the image-level labels. Two foreground classes are predicted: \textit{ActiveTuberculosis} and \textit{ObsoletePulmonaryTuberculosis}. The detected region centroids are intersected with a pretrained anatomical lung-zone segmentation~\cite{cohen2022torchxrayvision} to obtain an anatomical descriptor (lung side and zone) that the explanation module of Section~\ref{sec:explanation} consumes.

\subsubsection{Model Architecture}

We evaluate the \textbf{YOLO26} single-stage object detector and a variant, \textbf{YOLO26 with a DraxNet backbone}, in which the detector's default feature extractor is replaced by the DraxNet backbone used in the classification experiments. This isolates the effect of the backbone on TB localization while holding the detection head and training protocol fixed.
...
Table~\ref{tab:classification_results} reports image-level classification performance on the TBX11K test split. All models were trained from scratch without pretraining. Among the models with completed runs, the custom \textbf{DraxNet} achieves the strongest results, with the highest accuracy ($0.9897$) and TB-class AUC ($0.9990$), exceeding both the ResNet-18 and ResNet-50 baselines on every reported metric. ResNet-18 slightly outperforms the larger ResNet-50, suggesting the deeper baseline does not benefit the standardized CXR input at this data scale. \textbf{FlipR}, the lightweight symmetry-aware model, has only $2.2$M parameters---roughly one-fifth of ResNet-18---and its evaluation is still in progress; only its parameter count is reported here.

\begin{table}[!t]
\renewcommand{\arraystretch}{1.2}
\centering
\begin{threeparttable}
\caption{Image-Level TB Classification on the TBX11K Test Split}
\label{tab:classification_results}
\small
\begin{tabular}{@{}l c c c c c c@{}}
\toprule
\textbf{Model} & \textbf{Acc.} & \textbf{AUC$_{\text{TB}}$} & \textbf{Prec.} & \textbf{Rec.} & \textbf{F1} & \textbf{Params} \\
\midrule
ResNet-18 & 0.9841 & 0.9973 & 0.9727 & --- & --- & 11.7M \\
ResNet-50 & 0.9786 & 0.9965 & 0.9699 & --- & --- & 25.6M \\
DraxNet   & \textbf{0.9897} & \textbf{0.9990} & \textbf{0.9901} & --- & --- & 17.0M \\
FlipR     & --- & --- & --- & --- & --- & 2.2M \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\footnotesize
\item AUC$_{\text{TB}}$ is the area under the ROC curve for the TB class. Precision and recall are macro-averaged across the three classes.
\item All models were trained from scratch (no pretraining). A dash denotes a metric not yet available.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
Table~\ref{tab:detection_results} reports per-class bounding-box localization performance. Replacing the YOLO26 backbone with the DraxNet backbone improves \textit{ActiveTuberculosis} localization substantially, raising $\mathrm{mAP}_{50\text{-}95}$ from $0.2110$ to $0.2748$ ($+0.0638$), but slightly reduces \textit{ObsoletePulmonaryTuberculosis} from $0.0410$ to $0.0310$. The DraxNet backbone therefore helps the more salient active-disease class while marginally hurting the rarer, subtler obsolete-TB class. The low absolute $\mathrm{mAP}_{50\text{-}95}$ for \textit{ObsoletePulmonaryTuberculosis} is consistent with the known difficulty of localizing small, diffuse latent-TB regions in TBX11K. Precision, recall, and $\mathrm{mAP}_{50}$ are not yet available and are left blank.

\begin{table}[!t]
\renewcommand{\arraystretch}{1.2}
\centering
\begin{threeparttable}
\caption{Per-Class TB Localization Results}
\label{tab:detection_results}
\small
\begin{tabular}{@{}l l c c c c@{}}
\toprule
\textbf{Model} & \textbf{Class} & \textbf{Prec.} & \textbf{Rec.} & \textbf{mAP$_{50}$} & \textbf{mAP$_{50\text{-}95}$} \\
\midrule
YOLO26                    & ATB & --- & --- & --- & 0.2110 \\
YOLO26                    & OPT & --- & --- & --- & \textbf{0.0410} \\
\midrule
YOLO26 + DraxNet & ATB & --- & --- & --- & \textbf{0.2748} \\
YOLO26 + DraxNet & OPT & --- & --- & --- & 0.0310 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\footnotesize
\item ATB = ActiveTuberculosis; OPT = ObsoletePulmonaryTuberculosis. A dash denotes a metric not yet available.
\end{tablenotes}
\end{threeparttable}
\end{table}
