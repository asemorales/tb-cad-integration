...
The report generation module turns the detector's output into a structured radiology-style report a reader can act on without interpreting a boxed region or heatmap. That reader is a non-radiologist, in ACF often a radiographer, so the report is built around the triage and referral decision they make rather than a finding-level read. Example outputs are shown in Figures~\ref{fig:pipeline} and~\ref{fig:explanation_gallery}.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{../figures/cxr_report_gallery/cxr_report_gallery.png}
\caption{Example reports on TBX11K scans, one panel per case type, produced by the deterministic report module from the detector output. The ground-truth class is overlaid at the top-left of each radiograph and the ground-truth TB regions are boxed, active TB as a solid line and latent TB as a dashed line. All panels are correct cases, so the ground truth matches the report.}
