...
\begin{tablenotes}[flushleft]
\footnotesize
\item \textbf{In-dist} is the within-TBX11K AUC$_{\text{TB}}$ from Table~\ref{tab:classification_results}. \textbf{Zero-shot} applies the model as trained. \textbf{AdaBN} recomputes batch-normalization statistics on the unlabeled target images. AdaBN is not applicable to ConvNeXt-Tiny, which uses layer normalization rather than batch normalization. \textbf{Crop} crops the image to the lungs. Bold marks the best correction (among zero-shot, AdaBN, and crop) per model and dataset.
\end{tablenotes}
\end{threeparttable}
\end{table*}
...
\textbf{Zero-shot performance collapses on out-of-distribution data.}
...
\begin{figure}[!t]
    ...
    \caption{Binary TB ROC for the FlipR classifier on the three datasets. The dashed line marks the $90\%$ sensitivity operating point from WHO 2025 targets (minimum $40\%$ specificity at $90\%$ sensitivty)~\cite{who_cad_approval_2025}.}
    \label{fig:ood_roc}
\end{figure}
...
\textbf{Corrections recover model performance without retraining.}
...
\begin{tablenotes}[flushleft]
\footnotesize
\item Counts of bounding boxes. The 800 TBX11K TB images contain 1,211 boxes because a single image with TB may have multiple lesions.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{tablenotes}[flushleft]
\footnotesize
\item MACs is the number of multiply-accumulate operations for one forward pass. FLOPs are twice MACs.
\end{tablenotes}
\end{threeparttable}
\end{table*}
...
\caption{Examples of generated reports. The ground-truth class is overlaid at the top-left of each CXR. Radiologic laterality is used (i.e. lesions on the left side of the image are reported as appearing on the patient's right, and vice versa).}
\label{fig:explanation_gallery}
\end{figure*}

This module follows the two-stage detector-to-text architecture of Danu et al.~\cite{siemens_llm_cxr_2023}. However, because our detector emits a small, closed vocabulary rather than full radiographic findings, we replace their proprietary detector and fine-tuned language model with the public models of this paper and template-based report generation. Use of a generative model is reserved for future work that detects specific radiographic findings. For current use, template-based report generation suffices and does not require significant memory or compute for resource-constrained ACF settings.

The module has two stages: an adapter that converts detector output into a structured record, and a template-based report generator that renders that record as text. This architecture prioritizes faithfulness to model findings, because image-conditioned vision-language model architectures such as CXR-LLaVA~\cite{lee2024cxr_llava} \& LLaVA-Med~\cite{li2023llava_med} and generative multimodal models~\cite{siemens_llm_cxr_2023} may hallucinate findings the detector never flagged.

\subsection{Report Adapter}

The adapter maps classification and detection output to a fixed schema with a small, closed vocabulary. The image-level field carries the predicted label, the argmax over \textit{healthy}, \textit{sick non-TB}, and \textit{TB}, together with its class probabilities. Each detected region carries three fields: its type (\textit{active TB} or \textit{latent TB}), a confidence band based on the detection score (\textit{low} at $<0.5$, \textit{medium} at $<0.8$, or \textit{high} at $\geq0.8$), and a location. The location is assigned by mapping the bounding-box centroid to one of nine cells in a $3\times3$ grid over the image (upper, middle, or lower row; left, center, or right column). An image with no detections yields an empty region list.
...
The record is rendered into a structured template in three labeled sections, mirroring the findings-to-impression structure of a radiology report~\cite{zhang2024llm_impressions}: a numbered \textsc{findings} list with one TB region per line including lesion type, model confidence band, and lesion zone, an \textsc{impression} stating the screening result in clinical wording, and a \textsc{recommendation} based on the predicted class.

The report describes only the detected TB regions and does not name specific radiographic findings such as consolidation, cavitation, or fibrosis, nor cardiac, pleural, and skeletal observations. This is intended for non-radiologist readers in ACF (often radiographers), for whom chest X-ray interpretation falls outside the scope of practice and whose task is triage and referral rather than diagnosis. Specific findings that require radiological training to interpret may not be intuitive to non-radiologist readers, whereas the active-versus-latent distinction and recommendation that bears on referral urgency are what this non-radiologist reader can act on. Richer findings become useful when a radiologist receives the CXR later in a facility, which is reserved for future work.

We chose a template over a generative model after prototyping both. A language-model report generator (Phi-3.5-mini-instruct~\cite{abdin2024phi3} run on the record from the adapter) was prototyped alongside this work, but the target is a short, fixed-format report with one line per region, which a template reproduces exactly over the closed vocabulary. The template therefore matches the generative output on faithfulness, readability, and the ordering of findings. Since the generative approach does not provide additional benefit over the template, the generative model is dropped to avoid hallucination risk and reduce computational cost. A generative model would be needed for future work when the detector reports a richer, open set of interacting radiographic findings.

\subsection{Faithfulness by Design}

Report generation is faithful by design, meaning the generated text does not assert findings that are not present in the structured record. This is based on the faithfulness-versus-factuality distinction for abstractive summarization~\cite{maynez2020faithfulness} and data-to-text evaluation~\cite{dhingra2019parent}, under which a generation is faithful if every entity it states is supported by the source, independent of whether the source is itself correct. Faithfulness here is the absence of extrinsic hallucination. It does not by itself imply factual correctness, which is based on the accuracy of the upstream classification and detection models.
...
\begin{tablenotes}[flushleft]
\footnotesize
\item Measured on an Intel Core i7-8750H CPU using PyTorch with float32 compute, batch $1$, $512\times512$ input, and no GPU. \textbf{Size} is the size of the checkpoint on disk. \textbf{Lat.\ 1T} and \textbf{Lat.\ 6T} are the median single-image forward-pass latency at $1$ and $6$ CPU threads over $40$ runs. \textbf{RAM} is the peak RAM usage during the inference process, which includes a fixed runtime of about $330$ MB ($360$ MB with the detection library). These runtime figures are specific to this host and are not comparable across papers. To compare with related literature, see the parameters and MACs in Tables~\ref{tab:classification_results} and~\ref{tab:detection_results}.
\end{tablenotes}
\end{threeparttable}
\end{table}

To test on-device model performance, we benchmark each model on a commodity laptop CPU in Table~\ref{tab:ondevice}.

Laptops are typically used to run CAD systems in community-based ACF, resulting in limited compute. CAD is also increasingly deployed offline where network connectivity is unreliable or absent~\cite{stoptb_ultraportable, stoptb_cad_guide, min2025scanned}. Thus, models for community-based ACF should preferably be able to run locally on a CPU, without a dedicated GPU or a network connection.

\subsubsection{Memory Footprint}

On-disk size is primarily affected by parameter count. Our lightweight models fall at the parameter scale shown in related literature to run on CPU-only hardware. Lightweight chest X-ray networks have been deployed on edge devices and CPUs with low latency, including ResNet-18, MobileNet, and EfficientNet-B0 on an embedded board~\cite{benmalek2025edge} and a compact classifier on edge devices~\cite{chauhan2024covid_edge}, and for TB specifically LightTBNet reports rapid, low-memory prediction designed for embedded use~\cite{capellan2023lighttbnet}. LightTBNet ($1.3$M parameters), FlipR ($2.8$M), EfficientNet-B0 ($4.0$M), MobileNetV3-Large ($4.2$M), and Drax-MobileNetV3-Large ($4.8$M), and all tested stock YOLO detectors ($2.6\text{M-}3.2$M) are at or below the size of networks deployed on embedded boards in~\cite{benmalek2025edge, chauhan2024covid_edge}, so they should run on the same devices.

\subsubsection{Memory-Compute Tradeoff}

Tested classifiers tend to have a tradeoff between memory and compute. Although LightTBNet carries the lowest memory footprint ($1.3$M parameters; $5.1$MB size), it costs high compute ($14.1$G MACs). FlipR follows the pattern of lower memory fooprint ($2.8$M parameters; $11.2$MB size) with higher compute ($7.4$G MACs). Meanwhile, models with MobileNet backbones have lower compute requirements ($1.2\text{-}1.3$G MACs) but a higher memory footprint ($4.2\text{-}4.8$M parameters; $17.0-19.4$MB size). Hence, the classifier selected for deployment should be chosen based on whether memory or compute is more constrained in the target device. This is less relevant for detectors, which uniformly have higher memory footprint and lower compute.

\subsubsection{Inference Latency}

On-device latency is affected primarily by MACs. Every classifier completes a forward pass in under $0.7$ seconds on a single thread and under $0.26$ seconds on six threads.  The MobileNet backbones are fastest at about $90$ ms on a single thread. FlipR uses fewer parameters than MobileNet backbones but takes $244$ ms on a single thread. FlipR inference latency reduces significantly to $76$ ms on six threads because FlipR's standard convolutions parallelize across cores more efficiently than depthwise convolutions in MobileNet backbones. Despite LightTBNet using the fewest parameters, its high-resolution architecture gives it the third-highest single-thread latency at $621$ ms and fourth-highest six-thread latency at $214$ ms, similar to heavier models DraxNet, ResNet-50, and ConvNeXt-Tiny. Of the detectors, stock YOLO models have similar inference latency ($110\text{-}124$ ms single-thread, $46\text{-}68$ ms six-thread).

\subsubsection{RAM Usage}

RAM use is primarily based on activation size. All models' peak RAM usage fall within $600\text{-}820$ MB. Out of the classifiers, LightTBNet holds the fewest parameters yet the third-highest peak memory at $800$ MB, because its high-resolution feature maps dominate the footprint. FlipR, EfficientNet-B0, and models with MobileNet backbones show peak RAM usage around $603\text{-}655$ MB. Stock YOLO detectors show similar peak usage around $608\text{-}620$ MB.

\subsubsection{Selected Configuration}

For the abstract, conclusion, and cost analysis, we report figures for the FlipR-YOLOv8n models based on evaluation of performance and computational cost. Among the five classifiers with the lowest memory and compute cost (LightTBNet, FlipR, EfficientNet-B0, MobileNetV3-Large, Drax-MobileNetV3-Large), FlipR is selected because it achieves the highest zero-shot performance on external datasets Montgomery and Shenzhen. Among the three detectors with the lowest memory and compute cost (YOLOv8n, YOLO11n, YOLO26n), YOLOv8n is selected because it achieves the highest $\mathrm{mAP}$ scores for both active TB and latent TB. However, choice of deployment model for practical use should be chosen on the basis of constraints on the target device and/or community site.

\subsection{Deployment Cost}

We estimate representative costs on the selected configuration, FlipR-YOLOv8n, below. However, as deployment costs are highly variable based on implementation and deployment details, this cost analysis is intended to be an example estimate.

\subsubsection{Cost Estimate for Selected Configuration}

The selected configuration, FlipR-YOLOv8n, completes a CXR study in about $370$ ms on a single thread (Table~\ref{tab:ondevice}). A screening program of $100{,}000$ studies per year therefore needs about $10$ CPU-hours of compute per year. At a commodity cloud CPU rate near $0.045$ USD per core-hour, that is under $0.50$ USD per year, and on a single $300$ USD offline mini-PC amortized over a three-year service life, the same workload comes to about $110$ USD per year while leaving the device idle for most of the day. In community ACF, where the team already runs CAD on a laptop it owns~\cite{stoptb_cad_guide}, the marginal cost of an additional study is effectively only the electricity cost, because the pipeline is open source and does not require per-study license costs.

\subsubsection{Cost Estimate for Commercial TB CAD Systems}

Commercial TB CAD systems either have per-study or perpetual license costs, where prices are negotiated directly with health providers and are not published online~\cite{bashir2022economic}. The firmest public figures are the concessional bundle prices through the Stop TB Partnership Global Drug Facility, around $16{,}700$ USD for a perpetual CAD4TB license with hardware, installation, and one year of support. A multi-product cost model for Pakistan reports per-screen costs from $0.19$ to $2.73$ USD, where perpetual-license products approach a low per-screen cost only at high throughput and volume-priced products stay near the $0.93$ USD cost of a radiologist read~\cite{bashir2022economic}. Field programs report $13.31$ USD per screen for CAD4TB in Zambia~\cite{jo2021cad4tb} and a $9{,}000$ USD annual license alongside a $75{,}000$ USD imaging device for qXR in Nigeria~\cite{garg2025qxr}. However, these figures are highly dependent on the context of screening (i.e. base year, country-based pricing, currency exchange rates), so we report these figures as context rather than as a direct comparison against the estimated compute cost for FlipR-YOLOv8n above.

\subsubsection{Primary Factors for Deployment Costs}

Deployment cost is mainly driven by costs of TB CAD licensing, imaging hardware, and program logistics. As the pipeline is open source, it requires no per-study license costs, runs to completion on a commodity CPU without a GPU, and adds effectively zero marginal cost per study, which is advantageous for low-resource screening that the WHO identifies as the primary setting for TB CAD without radiologist readers~\cite{who_screening_2021}.

\section{Discussion}

\subsection{Limitations}

\subsubsection{Limited TB CXR Samples}

TB-specific samples are limited in public datasets. As of writing, TBX11K is the largest public CXR dataset for tuberculosis with labeled bounding boxes, yet its TB subset remains small at 800 TB images and 1,211 TB lesion boxes. While general CXR datasets exist, TB-positive samples are few and insufficient to train from scratch. No public dataset currently contains TB-specific CXRs with reference radiographic reports, which is what an evaluation of clinical correctness would require. This scarcity constrained the development and evaluation of report generation for clinical correctness.

\subsubsection{Evaluation on High-burden Populations}

The TB data scarcity limits how representative the evaluation population is. TBX11K is drawn from a single country, from patients presenting for hospital chest examination, and records no HIV status or prior-TB history. The WHO 2025 evaluation instead spans multiple regions, weighted toward high-burden African and South-East Asian settings, includes CXRs from community screening rather than facility screening, and reports lower CAD accuracy among women, people living with HIV, and those with a history of TB~\cite{who_cad_approval_2025}. Reported performance here may therefore overstate what the pipeline would achieve in the higher-burden, HIV-prevalent populations that community active case finding targets. Section~\ref{subsec:external_validation} measures one facet of this gap directly: zero-shot transfer to two other-source sets drops mean AUC$_{\text{TB}}$ from $0.998$ to $0.564$, and although label-free site adaptation recovers part of it, the corrected scores stay below the in-distribution figures. Evaluation on higher-burden, HIV-prevalent populations remains future work.

\subsubsection{Evaluation of Clinical Correctness}

Although report generation is faithful by design, faithfulness to the detector is not clinical correctness. A report that faithfully restates an incorrect detection is still wrong, and only reference-based or reader evaluation can report this. However, due to the lack of a TB-specific dataset with reference radiological reports, this is left to future work.

\subsubsection{Differences in Pediatric Presentation of TB}

The system's scope is limited to screening of adult patients because the radiographic presentation of pulmonary TB differs in pediatric patients. Adult reactivation tuberculosis appears in the upper lung zones, whereas pediatric primary tuberculosis appears in the lower lung zones. This is consistent with the WHO endorsement of CAD as a human-reader alternative only for patients aged 15 years and older~\cite{who_cad_policy_2021}. 

\subsection{Future Work}

\subsubsection{Evaluation on Field Hardware}

Future work seeks to extend the commodity-CPU benchmark reported in Table~\ref{tab:ondevice} to the target field hardware, measuring inference latency and memory on constrained single-board or ARM devices under real-world use.

\subsubsection{Detecting Full Radiographic Findings}
...
\subsubsection{Reader Study of Generated Reports}
...
TB CAD systems are highly accurate, however challenges remain in integrating them for community ACF with unreliable connectivity, limited compute, and no radiologist present. This paper presented a lightweight TB screening pipeline for resource-constrained community ACF settings. The selected FlipR-YOLOv8n configuration completes a study in about 370 ms on this CPU without a GPU. Because the pipeline is open-source, there is no per-study license and effectively zero marginal cost per study, as compared to commercial TB CAD systems with licensing and imaging hardware costs. For localization, a comparison of lightweight detectors covers both active and latent TB, the latter underdetected in prior work, Across all detectors, localization functions as a high-precision rule-in cue rather than a standalone detector. Among the classifiers, FlipR achieves similar accuracy and F1 to the strongest baselines with a parameter-efficient design and the second-highest zero-shot transfer to out-of-distribution data. However, zero-shot mean TB AUC falls from 0.998 to 0.564 on out-of-distribution data. Label-free site adaptation recovers model performance only slightly while staying below the in-distribution and WHO operating targets for TB CAD. The models are recommended to be trained on further data representative of high-risk populations and calibrate per community site as per WHO guidelines. TB-specific samples were found to be highly limited in public datasets, which constrained the design and evaluation of components. Evaluation of deployment feasibility on field hardware, extended radiographic findings for a full radiologist report, and a reader study with radiologists and radiographers is left to future work.
